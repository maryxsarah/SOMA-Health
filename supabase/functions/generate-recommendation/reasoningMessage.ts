...
/// 7.591666666667h) before it reaches ANY language's sleepHoursBit. Never
/// used for the underlying threshold comparisons (SHORT_SLEEP_HOURS etc. in
/// index.ts), only for display text.
function roundSleepHours(hours: number): number {
...
function supportingNumbers(t: Phrases, input: ReasoningMessageInput): string | null {
  const bits: string[] = [];
  if (input.sleepHours !== null) bits.push(t.sleepHoursBit(roundSleepHours(input.sleepHours)));
...
function lightCapExplanation(t: Phrases, input: ReasoningMessageInput): string | null {
  ...
  if (input.caps.sleep) {
    return t.lightCap.sleep(input.sleepHours !== null ? t.sleepHoursBit(roundSleepHours(input.sleepHours)) : "short sleep");
  }
